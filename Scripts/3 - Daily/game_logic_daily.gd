...
func _ready() -> void:
	...
	create_valid_answer_list()
	var date = Time.get_date_dict_from_system()
	var daily_index = date.year*365 + date.month*33 + date.day
	var word = valid_answers[(daily_index)%len(valid_answers)]
	word_to_guess = word
	set_keyboard_sprites()
	#word_to_guess = "march"
	print("Answer: ", word_to_guess)
...
func create_valid_answer_list():
	var file = FileAccess.open("res://Assets/possibleanswers.txt", FileAccess.READ)
	var content = file.get_as_text()
